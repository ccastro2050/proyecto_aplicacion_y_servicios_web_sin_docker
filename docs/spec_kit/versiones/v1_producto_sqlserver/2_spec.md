...
- **Arranque simple** (Artículo 4): `db/crear_bd.ps1` crea la BD en
  LocalDB (una vez) y `dotnet watch run` deja la API funcionando — sin
  Docker, sobre lo que trae Visual Studio.
...
1. **El arranque son dos comandos**: `.\db\crear_bd.ps1` deja la BD creada
   en LocalDB (12 tablas, 8 productos) y `dotnet watch run` (desde
   `api_facturas/`) deja la API corriendo; `GET http://localhost:8032/`
   responde el JSON de diagnóstico. Guardar un `.cs` recompila y reinicia
   solo (dotnet watch).
...
6. **Prueba de capas:** `dotnet run --project pruebas` (desde
   ...
   memoria — sin base de datos — y todas las verificaciones pasan.
